...
/**
 * PowerVmAllocationPolicyLeastMedianOfSquareRegression(LMS) VM allocation policy.
 */
public class PowerVmAllocationPolicyMigrationMestimation extends
	 ...
	 */
	public PowerVmAllocationPolicyMigrationMestimation(
	...
	protected boolean isHostOverUtilized(PowerHost host) {
		...
		try {
			upperThreshold = 1 - getSafetyParameter() * getHostUtilizationLMS(_host);
	...
	/**
	 * Gets the host utilization lms.
	 * 
	 * @param host the host
	 * @return the host utilization lms
	 */
	protected double getHostUtilizationLMS(PowerHostUtilizationHistory host) throws IllegalArgumentException {
		double[] data = host.getUtilizationHistory();
		if (MathUtil.countNonZeroBeginning(data) >= 12) { // 12 has been suggested as a safe value
			return MathUtil.mestimation(data);
		}
